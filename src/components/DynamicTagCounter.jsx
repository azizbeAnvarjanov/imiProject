...
const DynamicTagCounter = ({ collectionName }) => {
    ...
    const fetchTags = async () => {
      ...
      try {
        ...
        querySnapshot.forEach((doc) => {
          const data = doc.data();
          if (data.tag) {
            tags.push(data.tag); // Har bir jihozning `tag` qiymati
          }
        });
        ...
        const tagCountsPromises = uniqueTags.map(async (tag) => {
          const filteredQuery = query(itemsCollection, where("tag", "==", tag));
  ...
  return (
    <div className="w-full">
      <h1 className="font-bold text-xl mb-3">
        Jihoz taglari boyicha statistika
      </h1>
      {loading ? (
        <p>Yuklanmoqda...</p>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {tagsData.map(({ tag, count }) => (
            <div
              key={tag}
              className="py-2 px-2 border shadow-lg rounded-lg flex items-center justify-between"
            >
...
export default DynamicTagCounter;
